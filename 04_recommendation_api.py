# Filename: 04_recommendation_api.py (Upgraded to Sentence-BERT)
import pandas as pd
import joblib
...
from sentence_transformers import SentenceTransformer # <-- The new library
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Load the pre-computed semantic embedding matrices
        ...
        # Load the other artifacts (these are the same as before)
...
# We are using the new lifespan method to handle startup events
app = FastAPI(
    title="Course Recommendation API (v2 - Semantic)",
    lifespan=lifespan
)

# --- Pydantic Models for API Input/Output (no changes here) ---
...
# --- The Recommendation API Endpoint (with upgraded logic) ---
...
async def get_recommendations(request: RecommendationRequest):
    ...
    # --- NEW: Semantic Content-Based Scoring ---
    content_scores = {}
    ...
    for course_id in candidate_courses:
        ...
        # Give our new, powerful semantic model more weight in the final score
        hybrid_score = (0.7 * content_score) + (0.3 * normalized_cf_score)
        hybrid_scores[course_id] = hybrid_score

    # --- Rank and Return (logic is the same) ---
